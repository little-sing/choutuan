...
(function () {
  ...
  function openPayPanel(amount) {
    ...
    const ok = () => {
      ...
      const eggs = [];
      if (pwd === "071122") {
        eggs.push({ title: "这是本王的生日", body: "祝本王生日快乐!", btn: "好的" });
      }
      if (pwd === "071124") {
      ...
      if (djQty >= 3) {
        eggs.push({ title: "你想干嘛,外卖员都超载了", body: "一次点 3 个段嘉烁,外卖员承受不住", btn: "好的" });
      }
      /* 购买七安 */
  ...
  function renderTrack() {
      ...
      <div class="track">
        <div class="track-map">
          <svg viewBox="0 0 400 240" preserveAspectRatio="xMidYMid meet" style="width:100%;display:block">
            ...
            <text x="345" y="26" font-size="11" fill="#666" text-anchor="middle">翻斗花园六栋303</text>
    ...
    const moveRider = (prog) => {
      if (riderGroup && route && routeLen) {
        ...
        riderGroup.setAttribute("transform", `translate(${pt.x}, ${pt.y})`);
      }
    };
    moveRider(0);
